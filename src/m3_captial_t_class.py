...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        x1 = x+ width/2
        x2 = x- width/2
        y1 = y+ letter_thickness/2
        y2 = y- letter_thickness/2
        c1 = rg.Point(x1, y1)
        c2 = rg.Point(x2, y2)
        self.h_rect = rg.Rectangle(c1, c2)
        i1 = x + letter_thickness/2
        i2 = x - letter_thickness/2
        j1 = y - letter_thickness/2 + height
        j2 = y - letter_thickness/2
        p1 = rg.Point(i1, j1)
        p2 = rg.Point(i2, j2)
        self.v_rect = rg.Rectangle(p1, p2)
        # --------------------------------------------------------------
        # DONE: 3.
    ...
    def clone(self):
        ...
        fcolor = self.h_rect.fill_color
        ocolor = self.h_rect.outline_color
        t.set_colors(fcolor, ocolor)
        return t
        # --------------------------------------------------------------
        # DONE: 7.
        #   READ the above specification, including the Example.
